...
?>
	<?php
			...
			if(!empty($pge))
				{
					$reponse = $bdd ->prepare('SELECT * FROM kh_metier WHERE id_metier= ?');
					...
					while ($donnees = $reponse ->fetch())
					   {
							...
							$lie = $donnees['kh_services_id'];
							$stat = $donnees['statut'];

							$rpnse = $bdd ->prepare('SELECT * FROM kh_services WHERE id_service = ?');
							$rpnse ->execute(array($lie));
							while ($dones = $rpnse ->fetch())
							   {				  
									$services = $dones['libelle'];
								}
							$rpnse->closeCursor();
												...
												<div class="col-md-12">
													<section class="panel">														
														...
														<form class="form-validate form-horizontal " id="modif_form" method="post" action="modification.php">';
															...
															echo '<div class="form-group col-md-6">
																<label for="dac" class="control-label col-md-offset-2">Type de service: </label>
																<div class="col-md-offset-2">
																	<p>'.$services.'</p>
																	<select id="dac" class="form-control" name="serv" >
																		<option value="">Ne pas changer</option>';
																		$rpns = $bdd ->query('SELECT * FROM kh_services WHERE statut != 0');
																		while ($dons = $rpns ->fetch())
																		   {				  
																				$idcate = $dons['id_service'];
																				$nomcate = $dons['libelle'];
														...
														echo '<input type="hidden" name="modif" value="job"/>
