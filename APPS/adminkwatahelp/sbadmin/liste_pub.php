                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <?php echo $eng; ?> list
                            <a id="nouveau" class="btn btn-info" href="nvo_pub.php" title="Add a new"><i class="fa fa-edit fa-fw"></i> Nouveau</a>
                        </div>
                        <!-- /.panel-heading -->
                                            ...
                                            if(isset($_GET['rue']))
                                                {
                                                    if($_GET['rue'] == 1)
                                                        {
                                                            $reponse = $bdd ->query('SELECT * FROM kh_pub WHERE statut != 0 ORDER BY id_pub DESC');
                                                        }
                                                    elseif($_GET['rue'] == 2)
                                                        {
                                                            $reponse = $bdd ->query('SELECT * FROM kh_pub WHERE statut = 0 ORDER BY id_pub DESC');
                                                ...
                                                {           
                                                    $reponse = $bdd ->query('SELECT * FROM kh_pub  ORDER BY id_pub DESC');   
                                                }
                                            ...
                                            while ($donnees = $reponse ->fetch())
                                               {
                                                    $id = $donnees['id_pub'];
                                                    $img = $donnees['img_pub'];
                                                    $text = $donnees['text_pub'];
                                                    $rang = $donnees['priorite'];
                                                    $stat = $donnees['statut'];
                                                    ...
                                                    echo '<td><span class="photo">
                                                                <img id="affslide" alt="Avatar" src="../img/avatar/'.$img.'">
                                                            </span></td>';
                                                    echo '<td>'.$text.'</td>';
                                                    echo '<td>'.$text.'</td>';
                                                    echo '<td>'.$rang.'</td>';
