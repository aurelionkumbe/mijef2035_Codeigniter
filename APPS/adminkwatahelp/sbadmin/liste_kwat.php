                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <?php echo $eng; ?> list
                            <a id="nouveau" class="btn btn-info" href="nvo_kwata.php" title="Add a new"><i class="fa fa-edit fa-fw"></i> Nouveau</a>
                        </div>
                        <!-- /.panel-heading -->
                                            ...
                                            if(isset($_GET['rue']))
                                                {
                                                    if($_GET['rue'] == 1)
                                                        {
                                                            $reponse = $bdd ->query('SELECT * FROM kh_kwata WHERE statut != 0 ORDER BY nom_kwata DESC');
                                                        }
                                                    elseif($_GET['rue'] == 2)
                                                        {
                                                            $reponse = $bdd ->query('SELECT * FROM kh_kwata WHERE statut = 0 ORDER BY nom_kwata DESC');
                                                ...
                                                {           
                                                    $reponse = $bdd ->query('SELECT * FROM kh_kwata  ORDER BY nom_kwata DESC'); 
                                                }
                                            ...
                                            while ($donnees = $reponse ->fetch())
                                               {
                                                    ...
                                                    $cty = $donnees['kh_city_id'];

                                                    $stat = $donnees['statut'];
                                                    
                                                    $rponse = $bdd ->prepare('SELECT * FROM kh_city  WHERE id_city = ?');
                                                    $rponse ->execute(array($cty));
                                                    while ($donees = $rponse ->fetch())
                                                       {                  
                                                            $city = $donees['nom_city'];
                                                        }
                                                    $rponse->closeCursor();
